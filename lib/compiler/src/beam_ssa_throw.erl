...
si_handler_start(Is, Id, StartLbl, Last, Vars, Lst0, Gst) ->
    ...
    Handlers = case Suitability of
                   #{ Stacktrace := {tentative, EndLbl} } ->
                       ...
                       Partition = si_copy_blocks(Path, Blocks, #{}),
...
si_copy_blocks([Lbl | Lbls], Blocks, Acc0) ->
    Acc = Acc0#{ Lbl => map_get(Lbl, Blocks) },
    si_copy_blocks(Lbls, Blocks, Acc);
si_copy_blocks([], _Blocks, Acc) ->
    Acc.
