...
try_cg(Ta, SsaVs, Tb, SsaEvs, Th, BreakVars, St0) ->
    case try_cg_guard(Ta, SsaVs, Tb, Th, BreakVars, St0) of
        not_possible ->
            ...
            {Succ,St6} = new_ssa_var(St5),
            {Ais,St7} = cg(Ta, St6#cg{break=B,catch_label=H}),
            St8 = St7#cg{break=E,catch_label=St6#cg.catch_label},
            {Bis,St9} = cg(Tb, St8),
            {His,St10} = cg(Th, St9),
            {Handler,St11} = try_handler(H, TryTag, SsaEvs, St10),
            {KillTryTag,St} = kill_try_tag(TryTag, St11),
            {[#b_set{op=new_try_tag,dst=TryTag,args=[#b_literal{val='try'}]},
              #b_set{op={succeeded,body},dst=Succ,args=[TryTag]},
              #b_br{bool=Succ,succ=Next,fail=H},
              {label,Next}] ++ Ais ++
...
'catch_cg'(Block, #b_var{}=Dst, St0) ->
    ...
    {H,St3} = new_label(St2),
    {CatchTag,St4} = new_ssa_var(St3),
    {CatchSucc,St5} = new_ssa_var(St4),
    {OpSucc,St6} = new_label(St5),
    {Cis,St7} = cg(Block, St6#cg{break=OpSucc,catch_label=H}),
    {CatchedVal,St8} = new_ssa_var(St7),
    {SuccVal,St9} = new_ssa_var(St8),
    {CatchedAgg,St10} = new_ssa_var(St9),
    {CatchEndVal,St} = new_ssa_var(St10),
    Args = [#b_literal{val='catch'},CatchTag],
    {[#b_set{op=new_try_tag,dst=CatchTag,args=[#b_literal{val='catch'}]},
      #b_set{op={succeeded,body},dst=CatchSucc,args=[CatchTag]},
      #b_br{bool=CatchSucc,succ=Next,fail=H},
      {label,Next}] ++ Cis ++
         [{label,H},
          #b_set{op=landingpad,dst=CatchedAgg,args=Args},
          ...
          #b_set{op=catch_end,dst=Dst,args=[CatchTag,CatchEndVal]}],
