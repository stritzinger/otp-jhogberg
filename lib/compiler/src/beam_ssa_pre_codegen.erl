...
bs_restores([{L,#b_blk{is=Is,last=Last}}|Bs], CtxChain, D0, Rs0) ->
    ...
    {SuccPos, FailPos, Rs} = bs_restores_is(Is, CtxChain, StartPos, StartPos, Rs0),
...
bs_restores_is([#b_set{op=landingpad}|Is], CtxChain, SPos0, FPos0, Rs) ->
    ...
    SPos = maps:map(fun(_Start,_Pos) -> unknown end, SPos0),
    FPos = maps:map(fun(_Start,_Pos) -> unknown end, FPos0),
    bs_restores_is(Is, CtxChain, SPos, FPos, Rs);
