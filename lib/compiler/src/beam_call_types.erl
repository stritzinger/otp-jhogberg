...
will_succeed(erlang, Op, [LHS, RHS]) when Op =:= 'div';
    ...
    case {meet(LHS, #t_integer{}), meet(RHS, #t_integer{})} of
        {#t_integer{elements={_,_}}=LHS,
         #t_integer{elements={Div,_}}=RHS} when Div > 0 ->
            'yes';
        {#t_integer{}, #t_integer{}} ->
            'maybe';
        {_, _} ->
...
will_succeed(erlang, 'bsr', [LHS, RHS]) ->
    case {meet(LHS, #t_integer{}), meet(RHS, #t_integer{})} of
        {#t_integer{elements={_,_}}=LHS,
         #t_integer{elements={Shift,_}}=RHS} when Shift >= 0 ->
            'yes';
        {#t_integer{}, #t_integer{}} ->
            'maybe';
        {_, _} ->
...
will_succeed(erlang, 'bsl', [LHS, RHS]) ->
    case {meet(LHS, #t_integer{}), meet(RHS, #t_integer{})} of
        {LHS, #t_integer{elements={Shift,_}}=RHS} when Shift < 64 ->
...
will_succeed(erlang, byte_size, [Arg]) ->
    succeeds_if_type(Arg, #t_bitstring{});
will_succeed(erlang, element, [Pos, Tuple]) ->
    case meet(Tuple, #t_tuple{size=1}) of
        #t_tuple{exact=Exact,size=Sz}=Tuple ->
...
will_succeed(erlang, is_function, [_, Arity]) ->
    case meet(Arity, #t_integer{}) of
        #t_integer{elements={Min,_}}=Arity when Min >= 0 -> yes;
        #t_integer{} -> 'maybe';
        _ -> no
    end;
...
will_succeed(erlang, setelement, [Pos, Tuple, _Value]) ->
    case {meet(Pos, #t_integer{}),
          normalize(meet(Tuple, #t_tuple{size=1}))} of
...
succeeds_if_smallish(Number) ->
    case meet(Number, #t_integer{}) of
        #t_integer{elements={Min,Max}} when abs(Min) bsr 128 =:= 0,
                                            abs(Max) bsr 128 =:= 0 ->
            yes;
        _ ->
            'maybe'
...
types(erlang, Op, [LHS, RHS]) when Op =:= '<';
                                   Op =:= '=<';
                                   Op =:= '>=';
                                   Op =:= '>' ->
    case {LHS, RHS} of
        {#t_integer{elements=LHS_Range}, #t_integer{elements=RHS_Range}} ->
            case beam_bounds:relop(Op, LHS_Range, RHS_Range) of
                'maybe' ->
...
types(erlang, 'div', [_,_]=Args) ->
    ...
    sub_unsafe(beam_bounds_type('div', #t_integer{}, Args), ArgTypes);
types(erlang, 'rem', Args) ->
    ArgTypes = [#t_integer{}, #t_integer{}],
    sub_unsafe(beam_bounds_type('rem', #t_integer{}, Args), ArgTypes);
...
arith_type({bif,Op}, [_,_]=ArgTypes) when Op =:= '+';
                                          Op =:= '-';
                                          Op =:= '*' ->
    beam_bounds_type(Op, number, ArgTypes);
...
mixed_arith_types(Args0) ->
    [FirstType | _] = Args = [normalize(T) || T <- Args0],
    ...
    sub_unsafe(RetType, [number || _ <- Args]).
...
beam_bounds_type(Bounds, Type, [LHS, RHS]) ->
    case {meet(LHS, Type), meet(RHS, Type)} of
        {#t_integer{elements=Range1}, #t_integer{elements=Range2}} ->
            #t_integer{elements=beam_bounds:Bounds(Range1, Range2)};
        {none, _} ->
            none;
        {_, none} ->
            none;
...
lists_zip_types_1([none | _], _ListFun, _Es, _N) ->
    %% At least one of the lists is not a list
    {none, nil};
...
lists_zipwith_types(Fun, Types0) ->
    case meet(Fun, #t_fun{}) of
        #t_fun{type=ElementType} ->
            Types = [meet(T, #t_list{terminator=nil}) || T <- Types0],
