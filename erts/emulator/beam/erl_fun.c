 ...
 * a field other than its `HashBucket`. For example, starting it with an
 * `Export` whose entire address vector points to the function address will let
 * us use the same calling convention for both, reducing the cost of fun calls.
 * 
 * This will especially help once we have type-guided optimization in the
 * JIT: if we know that an instruction is only used with a fun of the correct
 * arity, the call can be reduced to two (!) instructions. */
typedef struct erl_fun_entry_container {
    /* !! MUST BE THE FIRST FIELD !! */
    HashBucket bucket;

    struct erl_fun_entry entry;
                    ...
                    const byte* uniq, int index, int arity)
{
    ...
    ASSERT(is_atom(mod));
    template.entry.old_index = old_index;
    template.entry.old_uniq = old_uniq;
...
const ErtsCodeMFA *erts_get_fun_mfa(const ErlFunEntry *fe) {
    ...
    ErtsCodePtr address = fe->export_entry.addresses[0];
...
int erts_is_fun_loaded(const ErlFunEntry* fe) {
    return fe->export_entry.addresses[0] != beam_unloaded_fun;
}
                              ...
                              struct erl_module_instance* modp)
{
    ...
    fun_addr = (const char*)fe->export_entry.addresses[0];
    mod_start = (const char*)modp->code_hdr;

    if (ErtsInArea(fun_addr, mod_start, modp->code_length)) {
        int i;

        fe->pend_purge_address = fe->export_entry.addresses[0];
        ERTS_THR_WRITE_MEMORY_BARRIER;

        for (i = 0; i < ERTS_ADDRESSV_SIZE; i++) {
            fe->export_entry.addresses[i] = beam_unloaded_fun;
        }
...
erts_fun_purge_abort_prepare(ErlFunEntry **funs, Uint no)
{
    ...
    for (ix = 0; ix < no; ix++) {
        ...
        if (fe->export_entry.addresses[0] == beam_unloaded_fun) {
            int i;

            for (i = 0; i < ERTS_ADDRESSV_SIZE; i++) {
                fe->export_entry.addresses[i] = fe->pend_purge_address;
            }
        }
    }
}
...
fun_alloc(ErlFunEntryContainer* template)
{
    ...
    for (i = 0; i < ERTS_ADDRESSV_SIZE; i++) {
        obj->entry.export_entry.addresses[i] = beam_unloaded_fun;
    }
